...
import BasketProduct from "./Components/NavBar/BasketProduct";
...
it('renders without crashing', () => {
  ...
  ReactDOM.render(
    <Shop products={PRODUCTS}/>, div);
});

it('check basket', () => {
  const newProducts = [...PRODUCTS];
  newProducts[0].inBasket = 1;
  newProducts[1].inBasket = 1;
  const wrapper = shallow(<Basket
    basket={newProducts.filter(p => p.inBasket)}
    removeFromBasket={function () {
    }}
  />);
  let length = wrapper.find("span").text();
...
it('hide category', () => {
  const wrapper = shallow(<Category
    ...
    expect(wrapper.find(".panel-body")).toEqual({});
